...
def main():
    ...
    data_file = next(
        (file for file in files if file.endswith("data_nettoyées.xlsx")), None)
    mp_file = next((file for file in files if file.endswith("MP.xlsx")), None)
    constraints_file = next(
        (file for file in files if file.endswith("Contraintes_composants.csv")), None)

    if data_file and mp_file and constraints_file:
        st.write(f"Loading files: {data_file}, {mp_file}, {constraints_file}")

        nuance = st.selectbox("Select the nuance", [
                              "GS 400-15", "GS 450-10", "GS 500-7", "GS 600-3"])
        faire_prediction = st.selectbox(
            "Faire prediction (oui/non)", ["oui", "non"])
    ...
    if st.button("Optimize"):
        ...
        with st.spinner("Running optimization..."):
            try:
                result = optimiser_nuance(
                    nuance, directory_path, faire_prediction == "oui")
