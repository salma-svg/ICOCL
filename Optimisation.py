...
def optimiser_nuance(nom_nuance, dossier_donnees, faire_prediction):
    # Import du chemin dunsolveur dans le fichier .ini
    config = configparser.ConfigParser()
    config.read(r"\\srvfil142.fdn.corp\Groupes\FDN_SOFT\AdrServeur.ini")
    chemin = config['PYTHON_SOLVER']['EXE']
    ...
    if 'GS 400-15' in nom_nuance or 'GS 400-18' in nom_nuance:
        ...
        seuil_rm = simpledialog.askstring("Input", "Veuillez entrer le seuil de résistance mécanique pour cette nuance :")
        seuil_all = simpledialog.askstring("Input", "Veuillez entrer le seuil d'allongement pour cette nuance :")
    ...
    while True:
        ...
        try :
            ...
            prob.solve(solver=COIN_CMD(path=chemin, msg = False)) # Utilisation du solveur CBC
